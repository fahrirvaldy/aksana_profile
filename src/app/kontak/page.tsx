...
export default function KontakPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-20">
      ...
      <div className="p-8 md:p-12 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm">
        {successMessage ? (
          <div className="text-center py-10 space-y-6">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-50 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
            </div>
            <p className="text-lg font-medium text-slate-900 dark:text-slate-50 leading-relaxed">
              {successMessage}
            </p>
            <button 
              onClick={() => setSuccessMessage(null)}
              className="text-sm font-semibold text-slate-500 hover:text-slate-900 dark:hover:text-slate-50 transition-colors"
            >
              Kirim pesan lain
            </button>
          </div>
